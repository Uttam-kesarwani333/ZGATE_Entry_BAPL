...
" Added by UTTAM for Quality
"Take Result
SELECT FROM i_insplotusagedecision AS a
       FIELDS
...
" If result recorded
       if lt_ins_dec is NOT INITIAL.
            ...
            elseif wa_ins_dec-InspectionLotUsageDecisionCode eq 'AD'.
                 ...
                 SELECT FROM zge_emal_comment AS a
                   FIELDS
                     a~gate_number,
                     a~pre_grn_qc,
                     a~approval_status
                   ...
                   INTO Table @DATA(lt_quality_appr).

" If no Entry of Approval
                    if lt_quality_appr is INITIAL.
                     READ TABLE gt_header ASSIGNING <fs_header> INDEX 1.
                  IF sy-subrc = 0 AND <fs_header> IS ASSIGNED.
                         <fs_header>-gate_status = 'Quality Email Pending'.
                          UNASSIGN <fs_header>.
                    ENDIF.

" Some Entry of Approval present whether Approved or DisApproved


                    ELSE.
                             read TABLE lt_quality_appr into DATA(wa_quality_appr) INDEX 1.
                             if wa_quality_appr-approval_status eq 'Not Approved'.
                                READ TABLE gt_header ASSIGNING <fs_header> INDEX 1.
                  IF sy-subrc = 0 AND <fs_header> IS ASSIGNED.
                         <fs_header>-gate_status = 'Gate Out Pending'.
                          UNASSIGN <fs_header>.
                    ENDIF.
                    ELSEif  wa_quality_appr-approval_status eq 'Approved'.
                       READ TABLE gt_header ASSIGNING <fs_header> INDEX 1.
                  IF sy-subrc = 0 AND <fs_header> IS ASSIGNED.
  ...
  METHOD get_new_gateid.
    ...
    IF use_number_range = abap_true.
      ...
      ELSEIF lv_type = 'Manual'.
        lv_nr_range_nr = 'G3'.
        ELSEIF lv_type = 'Bacardi' or lv_type = 'Bacardi' or lv_type = 'Radico' or lv_type = 'Pernod'.
        lv_nr_range_nr = 'G5'.
      ELSE.
        lv_nr_range_nr = 'G4'.
      ENDIF.
